...
viz1= st.expander("Click here for Histogram, Box-plot, Violin-plot Visualisations")
...
viz2= st.expander("Click here for Pie Chart and Heat-Map Visualisations")
...
weights,fats = st.columns([1,2])

new_title = '<p style="font-family:sans-serif; color:Pink; font-size: 20px;">Item Weight:</p>'
weights.markdown(new_title, unsafe_allow_html=True)
wei=weights.number_input("",value=2.0)

new_title = '<p style="font-family:sans-serif; color:Pink; font-size: 20px;">FAT TYPE:</p>'
fats.markdown(new_title, unsafe_allow_html=True)
fat=fats.selectbox("",["Low Fat","Regular"])

new_title = '<p style="font-family:sans-serif; color:Pink; font-size: 20px;">Visibility of the brand:</p>'
st.markdown(new_title, unsafe_allow_html=True)
visible= st.slider("",min_value=0.0,max_value=1.0,value=0.1)

new_title = '<p style="font-family:sans-serif; color:Pink; font-size: 20px;">Item Categories</p>'
st.markdown(new_title, unsafe_allow_html=True)
...
mrps,out_ids,years = st.columns([2,2,2])

new_title = '<p style="font-family:sans-serif; color:Pink; font-size: 20px;">MRP of the item:</p>'
mrps.markdown(new_title, unsafe_allow_html=True)
mrp=mrps.number_input("",value=1.0)

new_title = '<p style="font-family:sans-serif; color:Pink; font-size: 20px;">Outlet Identifier code:</p>'
out_ids.markdown(new_title, unsafe_allow_html=True)
out_id=out_ids.selectbox("",['OUT027', 'OUT013', 'OUT046', 'OUT049', 'OUT035', 'OUT045', 'OUT018', 'OUT017', 'OUT010', 'OUT019'])

new_title = '<p style="font-family:sans-serif; color:Pink; font-size: 20px;">Outlet Est. Year:</p>'
years.markdown(new_title, unsafe_allow_html=True)
year=int(years.number_input("",value=1988))

ots,olocs = st.columns([3,2])

new_title = '<p style="font-family:sans-serif; color:Pink; font-size: 20px;">Outlet type (size of export):</p>'
ots.markdown(new_title, unsafe_allow_html=True)
ot=ots.selectbox("",['Small','Medium','High'])

new_title = '<p style="font-family:sans-serif; color:Pink; font-size: 20px;">Output Location Type</p>'
olocs.markdown(new_title, unsafe_allow_html=True)
oloc=olocs.radio("",['Tier 1', 'Tier 2', 'Tier 3'])

new_title = '<p style="font-family:sans-serif; color:Pink; font-size: 20px;">Store Type</p>'
st.markdown(new_title, unsafe_allow_html=True)
